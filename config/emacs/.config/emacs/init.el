;;; init.el --- Configuration primary entry point -*- lexical-binding: t; -*-
...
;; Load our early-init if Emacs is too old to do so automatically
...
;; Redirect native generated code cache
...
(require 'hgs-straight)
(hgs-straight-bootstrap)
(eval-when-compile
  (require 'use-package))

;; Ensure & activate external packages
...
;; Personal functions
(defun hgs-indent-current-buffer ()
...
(defun hgs-byte-compile-configuration ()
  ...
  (interactive)
  (cl-loop with files = (directory-files-recursively hgs-emacs-config-directory
                                                     ".*.el$")
           for f in files
           unless (member (file-name-sans-extension (file-name-base f))
                          '("lock"
                            "customization"))
           do (byte-recompile-file f nil 0)))

(defun hgs-restart-emacs ()
  "Restart our Emacs (if possible)."
  (interactive)
  ;; Should be available from Emacs 29
  (if (fboundp #'restart-emacs)
      (restart-emacs)
    (message "Built-in `restart-emacs' is not available!")))

(defun hgs-get-line-info-current-buffer ()
  "Add the current buffer line information to the kill ring as FILE:LINE."
  (interactive)
  (let ((position-info
         (if (buffer-file-name)
             (format "%s:%d" (buffer-file-name)
                     (line-number-at-pos))
           ;; If we aren't in a file-visiting buffer, the current directory is
           ;; a decent default
           default-directory)))
    (message "Yanked buffer line information '%s' to the kill ring."
             position-info)
    (kill-new position-info)))
...
(defun hgs-delete-custom-file ()
  ...
  (interactive)
  (cl-loop when custom-file
           with exts = '(".el" ".elc" ".eln")
           for ext in exts
           collect (concat (file-name-sans-extension custom-file)
                           ext) into fs
           finally (cl-loop for f in fs
                            do (message "Deleting %s" f)
                            (delete-file f))))
...
(use-package elec-pair
  ...
  :hook
  ;; TODO: Consider enabling for all modes by default
  ((scheme-mode lisp-mode emacs-lisp-mode) . electric-pair-local-mode))
...
(use-package kkc
  ...
  :custom
  (kkc-init-file-name (concat hgs-emacs-config-directory "kkc/init.el")))
...
careful about not doing this if we are in daemon mode though,
...
(use-package treesit-auto
  :ensure nil
  :demand t
  ;; treesit.el is an integrated part of emacs >= 29
...
(use-package puni
  ...
  :hook
  ;; TODO: Consider whether to enable in all text/prog modes
  ((scheme-mode lisp-mode emacs-lisp-mode) . puni-mode)
  ((shell-mode term-mode vterm-mode) . puni-disable-puni-mode)

  :bind
  (:map puni-mode-map
        ("C-M-t" ("Transpose sexps" . puni-transpose))
        ("C-M-y" ("Convolute sexps" . puni-convolute))
        ("C-)" ("Forward slurp sexp" . puni-slurp-forward))
        ("M-)" ("Forward barf sexp" . puni-barf-forward))
        ("C-(" ("Backward slurp sexp" . puni-slurp-backward))
        ("M-(" ("Backward barf sexp" . puni-barf-backward))
        ("M-D" ("Splice sexp" . puni-splice))
        ("M-S" ("Split sexp" . puni-split))
        ("M-R" ("Raise sexp" . puni-raise))
        ;; Replace 'mark-sexp'
        ("C-M-SPC" ("Expand region" . puni-expand-region)))
...
(use-package dashboard
  ...
  (dashboard-set-footer nil "Disable the randomly selected footnote.")
...
(load-if-exists (concat hgs-emacs-config-directory
                        "init-local"))

(load-if-exists custom-file)

;; Unneeded, but to silence linting
;; (provide 'init)
